...
<body> 
	...
	<body>
		...
		<?php if (!isset($_SESSION["user"])){ ?>
			...
			<?php } else { ?>
		<div class="col-xs-12">
				<form class="form-horizontal" id="formbuscador" method="post" action="maestrocliente.php">
		...
		<div class="datos" id="articulos">
			...
			$rs =  mysqli_query($conectado,"SELECT CodiUsua, MailUsua, EmprUsua, NombUsua, ApelUsua, EstaUsua FROM usuario WHERE NombUsua LIKE '%".$txt_criterio."%' OR  ApelUsua LIKE '%".$txt_criterio."%' OR  EmprUsua LIKE '%".$txt_criterio."%' OR  EstaUsua LIKE '%".$txt_criterio."%'");

				$producto =  mysqli_query($conectado,"SELECT CodiUsua, MailUsua, EmprUsua, NombUsua, ApelUsua, EstaUsua FROM usuario WHERE NombUsua LIKE '%".$txt_criterio."%' OR  ApelUsua LIKE '%".$txt_criterio."%' OR  EmprUsua LIKE '%".$txt_criterio."%' OR  EstaUsua LIKE '%".$txt_criterio."%' LIMIT ".$inicio. ",".$TAMANO_PAGINA."");
			/*
			...
			*/
			...
			$total_paginas = ceil($num_total_registros / $TAMANO_PAGINA); 
			 ...
			 while( $prod = mysqli_fetch_array($producto) ) {
    			?>
    			<tr>
    			<?php $id = $prod["CodiUsua"]; ?>
      			
      			<td><?php echo $prod["MailUsua"] ; ?></td>
      			<td><?php echo $prod["ApelUsua"] ; ?></td>
      			<td><?php echo $prod["NombUsua"] ; ?></td>
      			<td><?php echo $prod["EmprUsua"] ; ?></td>
      			<td><?php echo $prod["EstaUsua"] ; ?></td>
      			<td><?php echo "<a class='page-link' href='modificarcl.php?id=" . $id . "'><img src='imagen/modificar.png' border='0' width='20' class='img-responsive'></a>"; ?></td>
      			<td><?php echo "<a class='page-link' href='delete.php?id=" . $id . "'><img src='imagen/eliminar.png' border='0' width='20' class='img-responsive'></a>"; ?></td>
      			</tr>			
		        <?php }
				?>
			  </tbody>
		...
		if ($total_paginas > 1){
		    for ($i=1;$i<=$total_paginas;$i++){
		       if ($pagina == $i)
		          ...
		          echo "<li class='page-item'><a class='page-link' href='maestrocliente.php?pagina=" . $i . " &criterio=".$txt_criterio."'>" . $i . "</a></li> ";
		    }
		} ?>
